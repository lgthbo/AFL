...
struct queue_entry {
  ...
  u8* fname;                          /* File name for the test case 檔名 */
  u32 len;                            /* Input length        testcase大小 */

  u8  cal_failed,                     /* Calibration failed?   校準失敗?   */
      trim_done,                      /* Trimmed?  該testcase是否被修剪過  */
      was_fuzzed,                     /* Had any fuzzing done yet?        */ /* 是否有被fuzz 過 needed for MOpt  */
      passed_det,                     /* Deterministic stages passed?     */
      has_new_cov,                    /* Triggers new coverage?           */
      var_behavior,                   /* Variable behavior?               */
      favored,                        /* Currently favored?               */ /* 當前是否被標記為favored(更多的fuzz機會) */
      fs_redundant;                   /* Marked as redundant in the fs?   */

  u32 bitmap_size,                    /* Number of bits set in bitmap     */ /* bitmap中bit的數量 */
      exec_cksum;                     /* Checksum of the execution trace  */

  u64 exec_us,                        /* Execution time (us)              */ /* 執行時間延遲 */
      handicap,                       /* Number of queue cycles behind    */
      depth;                          /* Path depth                       */ /* 路徑深度 */

  u8* trace_mini;                     /* Trace bytes, if kept             */ /* 1個bit存一個byte的trace_mini */
...
/*
構建繫結到特定核心的程序列表 找不到的話會回傳 -1 假設有一個4核心的cpu
*/
...
/*
*  將新的測試用例插入佇列，並初始化fname檔名稱，
*  增加cur_depth深度++ queued_paths測試用例數量++
*  pending_not_fuzzed沒被fuzzed測試用例數量++，
...
/* Read all testcases from the input directory, then queue them for testing.
   Called at startup. */
   /*
   * 將in_dir目錄下的測試用例掃描到queue中，
   * 並且區分該檔案是否為經過確定性變異的input，
   * 如果是的話跳過，以節省時間。
   */
...
static void read_testcases(void) {
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    add_to_queue(fn, st.st_size, passed_det); // 將測試用例排成 Queue
...
/*
儲存命令列引數  會將 orig_cmdline 指標變數指向某個函式
*/
...
int main(int argc, char** argv) {
  ...
  setup_signal_handlers(); /* 設定訊號控制代碼 */
  check_asan_opts(); /* 檢查ASAN選項 */
  ...
  /*
  *  檢查是否微dumb mode 模式以及crash mode 和 qemu mode 是否聯動設定
  */
  ...
  save_cmdline(argc, argv); // 儲存命令列引數

  fix_up_banner(argv[optind]); // 修剪並建立執行用的banner

  check_if_tty(); // 檢查是否在tty終端上執行

  get_core_count(); //取得核心數量

#ifdef HAVE_AFFINITY
  bind_to_free_cpu(); // 構建繫結到特定核心的程序列表
#endif /* HAVE_AFFINITY */
  ...
  setup_dirs_fds(); // 設定輸出目錄及檔案描述符
  read_testcases(); // 讀取測試檔案
  load_auto();
